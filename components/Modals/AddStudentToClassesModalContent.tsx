...
interface AddStudentToClassesModalContentProps {
	idStudent: number;
...
const AddStudentToClassesModalContent = ({ idStudent, existingClassIds }: AddStudentToClassesModalContentProps) => {
		...
		try {
			const getClasses = async () => {
				...
				if ('success' in res) {
					...
					console.error('AddStudentToClassesModalContent:Error::', 'message' in res ? res.message : 'Unknown error');
	...
	const handleSubmit = async () => {
		try {
			...
			// Ajouter l'étudiant à toutes les classes sélectionnées
			const promises = Array.from(selectedClassIds).map((idStr) => putStudentToClass(Number(idStr), idStudent));
			...
			if (!allSuccess) {
				...
				closeModal();
				customToast.success(selectedClassIds.size > 1 ? `Étudiant ajouté à ${selectedClassIds.size} classes avec succès` : 'Étudiant ajouté à la classe avec succès');
	...
	return (
		<>
			<ModalHeader className="flex flex-col gap-1">
				<div>Ajouter l&apos;étudiant à des classes</div>
			</ModalHeader>
...
export default AddStudentToClassesModalContent;
